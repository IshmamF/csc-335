; Problem: Write a procedure lexical-address that takes any expression and returns the expression with every variable reference v replaced by a list (v : d p), as above.
; If the variable reference v is free, produce the list (v free) instead.
; Example:
        ...
        (if (symbol? exp)
            ...
            (let* ((add-params (cons (second exp) env)))
              (cond
                ...
                ((check-if exp)(make-if (helper env (second exp)) (helper env (third exp)) (helper env (fourth exp))))
